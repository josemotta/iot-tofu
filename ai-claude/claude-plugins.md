...
Create a folder at ~/.claude/skills/<name>/ and add a SKILL.md with frontmatter (name, description) and a body of instructions. Optionally add supporting files like templates or example code. The next Claude Code session will discover the skill and apply it when the description matches the current task. The description field is the most important part of a skill. It is what the model reads to decide whether to apply the skill.
...
Encore's static infrastructure analysis, strong TypeScript/Go backend framework integration, and tight local feedback loops allow AI coding agents to write, validate, and wire up complex backend architectures and databases instantly with far fewer hallucinations. LoopBack 4 is heavy, heavily reliant on complex decorators and boilerplate, and largely legacy compared to modern cloud-native meta-frameworks.

### Why Encore Wins for AI Development

- AI-Friendly Structure: Encore natively supports .cursorrules and static context that help AI agents completely understand service boundaries, database schemas, and endpoints.

- No Infrastructure Boilerplate: AI can write the business logic while Encore automatically handles infrastructure wiring (PostgreSQL, Pub/Sub, and local Docker management).

- Immediate Feedback: Instant local validation lets AI agents test and self-correct errors in a tight loop before deploying.

### Why LoopBack 4 Falls Short
