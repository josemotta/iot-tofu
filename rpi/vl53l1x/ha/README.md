...
This is an example of a typical [configuration](configuration.yaml) file, expected to be **installed at RPIs** with proper sensor, not at Tofu boot server. It is also accessing raw sensors data from files as part of the sysfs interface. The Homeassistant screenshot for the Time-of-Flight (ToF) laser-ranging sensor is shown below.
